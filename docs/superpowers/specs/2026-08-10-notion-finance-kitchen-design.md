...
### 5.3 營養:內建粗估表,不接任何外部 API

**決定:完全不接外部營養資料源**(既不用食藥署 CSV,也不用 USDA API)。

理由:營養值本來就因品種、部位、產地而異,線上查詢換來的精度對「今天該煮什麼」沒有幫助,卻多一個會壞的相依與一組要管的金鑰。精準值由使用者自行查證後手動修改 Notion。

**做法:** `kitchen.py` 內建約 26 種常買食材的每 100g 粗估值,加上單位→克數的換算表:

1. `estimate_grams(name, qty, unit)` — 重量單位直接換算;個數單位查個別典型單重,查無則退到分類層級的兜底值
2. `lookup_nutrition(name)` — 回每 100g 值,支援包含比對(「有機高麗菜」→「高麗菜」)
3. Notion 只存**每 100g 值 + 重量克**,總熱量用 formula `round(prop("熱量") * prop("重量克") / 100)` 算 —— 使用者改重量,總量自己更新

**估不出來一律留 None**,欄位留空。編一個克數出來會讓整筆營養都錯,留空反而誠實。
每筆標記 `營養為粗估` checkbox,使用者查證後可自行取消勾選。
