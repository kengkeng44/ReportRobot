...
**已部署於 Railway。測試數 2026-09-05 為 1079 個**(這份文件第 2 節那張表
只記到首次部署那批 15 個 commit,之後的功能看第 3 節的驗證狀態表)。
...
- [ ] **語句庫要人工填內容**(2026-09-04)。部署後 `ensure_all_dbs()` 會建出
  「📚 語言學習」區塊與「語句庫」「金句庫」兩張空表。**這兩張不用手動接權限**——
  機器人在既有的 parent page 底下建的,integration 對子頁面自動有權限。
  把 Preply 老師整理的句子貼進「語句庫」就好,貼進去當天就會出現在信裡
...
- [ ] **金句搬遷**(2026-09-04)。舊的「每日一句」370 句要匯進金句庫:
  ...
  > ⚠️ **舊的「每日一句」那張表才是要手動接權限的**:它是從
  > renhezheng44 帳號匯出 CSV 再匯入的,不是機器人建的,所以沒有繼承權限。
  > 對那一頁按 **Share → Connect**,不然腳本會讀到 0 筆。
  >
