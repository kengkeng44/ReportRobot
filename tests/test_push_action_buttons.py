...
def _kitchen_bubble(msg):
    contents = msg["contents"]
    bubbles = contents["contents"] if contents.get("type") == "carousel" else [contents]
    for b in bubbles:
        if b["header"]["contents"][0]["text"] == "🥬 食材提醒":
            return b
    return None


def test_carousel_kitchen_bubble_has_buttons_when_items_given():
    items, _ = kitchen.expiring_actions(_pantry(("菠菜", 1), ("板豆腐", 2)))

    msg = flex_builder.daily_report_carousel(
        extra_text="小知識", weather_text="晴天", premarket_text="盤前",
        today_str="2026-08-13", kitchen_items=items,
    )

    assert len(_buttons(_kitchen_bubble(msg))) == 2


def test_carousel_kitchen_bubble_still_before_weather():
    items, _ = kitchen.expiring_actions(_pantry(("菠菜", 1)))
...
def test_carousel_falls_back_to_text_bubble_without_items():
    """定位不到 page_id 時仍要有提醒，只是沒按鈕。"""
    msg = flex_builder.daily_report_carousel(
        extra_text="小知識", weather_text="晴天", premarket_text="盤前",
        today_str="2026-08-13", kitchen_text="⚠️ 菠菜快過期", kitchen_items=[],
    )

    assert "🥬 食材提醒" in _titles(msg)
    assert _buttons(_kitchen_bubble(msg)) == []
...
def test_payload_gives_items_for_buttons(use_notion):
    use_notion(pantry=_pantry(("菠菜", 1), ("米", 300)))

    payload = daily_report._kitchen_payload()

    assert [i["name"] for i in payload["items"]] == ["菠菜"]


def test_payload_separates_recipe_text_from_expiring_list(use_notion):
    """有按鈕時，快過期清單由按鈕列呈現，bubble 文字只留建議菜色。"""
    use_notion(
    ...
    assert "菠菜豆腐湯" in payload["recipe_text"]
    assert "菠菜豆腐湯" in payload["text"], "純文字 fallback 仍要完整"


def test_payload_is_none_when_nothing_expiring(use_notion):
    use_notion(pantry=_pantry(("米", 300)))
    assert daily_report._kitchen_payload() is None
