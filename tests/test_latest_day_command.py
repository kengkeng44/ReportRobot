...
def test_group_carousel_cannot_carry_spending():
    """群組推播看不到財務。

    2026-08-19 之前這裡斷言 daily_report 沒有 _spending_recent 屬性,
    但個人版(推到本人 1 對 1)把消費加回來了,那個函式又存在了。
    真正要守的是「財務不進群組」,不是「某個函式不存在」——
    所以改成檢查群組版的卡片組裝函式結構上就收不了消費。
    ...
    params = inspect.signature(daily_report_carousel).parameters
